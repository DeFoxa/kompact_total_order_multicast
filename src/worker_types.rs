...
// **NOTE**(Fix in progress): current worker state update handling is incorrect in async environment. We need to
...
//TODO (In Progress: handle_accepted_proposal, queue_message_for_delivery): write logic to handle accepted proposal from master: verify if
//worker = owner of accepted proposal, if yes mark deliverable, update state using message content, log logical_time and
...
impl Worker {
    ...
    fn queue_message_or_deliver(
        ...
        if let Some(last_delivered) = self.delivered_messages.iter().max_by_key(|p| p.0) {
            if last_delivered.0.time + 1 == logical_time.time {
                ...
                self.process_queue_for_sequential_deliverables(LamportClock {
    ...
    fn process_queue_for_sequential_deliverables(
        &mut self,
        logical_time: LamportClock,
    ) -> Result<()> {
